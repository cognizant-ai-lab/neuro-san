...
class RegistryManifestRestorer(Restorer):
    ...
    def derive_mcp_tool_name(network_name: str, manifest_dict: Dict[str, Any]) -> str:
        ...
        if not tool_name:
            tool_name = McpToolNamePolicy.to_tool_name(network_name)

        if not McpToolNamePolicy.is_valid_tool_name(tool_name):
        ...
        if McpToolNamePolicy.is_over_soft_limit(tool_name):
