...
class LangChainMcpAdapter:
    ...
    "basic__music_nerd_pro" via McpToolNamePolicy.to_tool_name(). Only the
    ...
    async def get_mcp_tools(
        ...
        original names), then renamed with McpToolNamePolicy.to_tool_name() so the
    ...
    def _filter_allowed_tools(
        ...
        unmatched: List[str] = McpToolNamePolicy.find_unmatched(client_allowed_tools, original_names)
        if unmatched:
            self.logger.warning(
        ...
        selected: List[str] = McpToolNamePolicy.select_allowed(client_allowed_tools, original_names)
    ...
    @staticmethod
    def _collect_safe_names(mcp_tools: List[BaseTool]) -> Set[str]:
        """
        ...
        :return: The set of tool names that McpToolNamePolicy.to_tool_name() leaves unchanged.
        """
        safe_names: Set[str] = set()
        for tool in mcp_tools:
            if McpToolNamePolicy.to_tool_name(tool.name) == tool.name:
    ...
    def _rename_tools_for_providers(self, server_url: str, mcp_tools: List[BaseTool]) -> List[BaseTool]:
        ...
        safe name collides with another tool from the same server.
        ...
        for tool in mcp_tools:
            safe_name: str = McpToolNamePolicy.to_tool_name(tool.name)
            ...
            self.logger.info(
                ...
                server_url, tool.name, safe_name)
            if McpToolNamePolicy.is_over_soft_limit(safe_name):
                # "__" is longer than "/", so a rename can push a name past the
                # OpenAI cap. Anthropic and local providers accept it, so warn only.
                self.logger.warning(
                    "MCP server %s: renamed tool '%s' is %d characters long, over the %d-character "
