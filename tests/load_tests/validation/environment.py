...
from tests.load_tests.monitoring.resource_monitor import find_process
from tests.load_tests.monitoring.resource_monitor import find_process_by_port
...
def find_local_server(args):
    ...
    for keyword in ["neuro_san_studio", "server_main_loop"]:
        server_proc = find_process(keyword)
    ...
    if server_proc is None:
        server_proc = find_process_by_port(args.port)
        if server_proc is not None:
            logger.info(
    ...
    if server_log is None:
        server_log = _auto_detect_server_log(server_proc)
