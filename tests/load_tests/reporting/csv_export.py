...
def export_per_request_csv(output_dir, stage_summaries, agent_name):
    ...
    # Collect all unique parsed field names from results
    parsed_field_names = set()
    for summary in stage_summaries:
        for result in summary.get("results", []):
            for key in result:
                if key not in {"status", "elapsed", "prompt", "error", "request_id"}:
        ...
        for summary in stage_summaries:
            ...
            for result in summary.get("results", []):
                ...
                row = {
                    ...
                    "stage": stage,
                    "round": round_num,
                    "request_id": result.get("request_id", f"request-{request_num}"),
                    "status": result.get("status", ""),
                    "duration_sec": f"{result.get('elapsed', 0):.2f}",
...
def export_summary_csv(output_dir, stage_summaries, agent_name):
    ...
    if has_tokens:
        fieldnames.extend([
            "total_tokens", "avg_tokens", "p50_tokens", "p90_tokens",
            "max_tokens", "total_prompt_tokens", "total_completion_tokens",
    ...
    if has_tokens:
        ...
        row.update({
            ...
            "p50_tokens": int(_percentile(token_totals, 50)),
            "p90_tokens": int(_percentile(token_totals, 90)),
...
def append_resource_history(stage_summaries, resource_rows, client_rows):
