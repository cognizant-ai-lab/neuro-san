...
def log_recommendations(stage_summaries, args=None, output_dir=None):
    """Log actionable recommendations and write to file."""
    recommendations = _detect_issues(stage_summaries)
    observations = _summarize_observations(stage_summaries)
    suggestion = _suggest_next_step(stage_summaries, args)
    ...
    if recommendations:
        ...
        total_reqs = sum(s.get("concurrent", 0) for s in stage_summaries)
        max_concurrent = max(
            (s.get("concurrent", 0) for s in stage_summaries), default=0,
        )
        lines.append(
            f"  No issues detected at current load "
            f"({total_reqs} requests, max {max_concurrent} concurrent).",
        )
        lines.append("")
    ...
    if output_dir:
        _write_recommendations_file(output_dir, lines)
...
def _detect_issues(stage_summaries) -> List[str]:
    ...
    failed = sum(
        ...
        if r.get("status") in (STATUS_FAILED, STATUS_TIMEOUT, STATUS_KILLED)
    )
    if total > 0 and failed > 0:
        fail_pct = failed / total * 100
        recommendations.append(
            f"FAILURES DETECTED: {failed}/{total} requests failed "
            f"({fail_pct:.0f}%).\n"
            f"     Review per-request CSV for failure details and "
            f"server log for errors."
        )
...
def _summarize_observations(stage_summaries) -> List[str]:
    ...
    # Failure/timeout/retry counts
    created = sum(1 for r in all_results if r.get("status") == STATUS_CREATED)
    failed = sum(1 for r in all_results if r.get("status") == STATUS_FAILED)
    timed_out = sum(
        1 for r in all_results if r.get("status") == STATUS_TIMEOUT
    )
    total_retries = sum(s.get("total_retries", 0) for s in stage_summaries)
    observations.append(
    ...
    for s in stage_summaries:
        ...
        if results:
            avg = sum(r.get("elapsed", 0) for r in results) / len(results)
    ...
    if stages_with_resources:
        ...
        # RSS info is in resource rows but we can infer from thread data
    ...
    if len(stages_with_resources) >= 2:
        for s in stages_with_resources:
            ...
            new_t = max(s.get("after_threads", 0) - s.get("before_threads", 0), 0)
    ...
    if token_totals:
        ...
        models = set(r.get("model") for r in all_results if r.get("model"))
...
def _suggest_next_step(
    ...
    agent = getattr(args, "agent", "hello_world") if args else "hello_world"
    is_ramp = getattr(args, "ramp", False) if args else False
    stages_used = []
    for s in stage_summaries:
    ...
    # Build extra flags from current args
    extra_flags = []
    if args:
    ...
    # Flat mode — suggest switching to ramp
    max_workers = max(stages_used) if stages_used else 2
    ramp_stages = [max_workers, max_workers * 2, max_workers * 4]
...
def _write_recommendations_file(output_dir, lines):
    ...
    with open(filepath, "w", encoding="utf-8") as fh:
        for line in lines:
            # Strip the leading padding used for console alignment
            fh.write(line.strip() + "\n")
